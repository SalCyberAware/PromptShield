...
- **OpenAI is the third judge, after Claude and Gemini, but never against its own family.** The product cascade and the eval chain both add it only when the target is outside the OpenAI family (`model_config.judge_excluded_for_target`; an unknown target counts as OpenAI). With the web demo's default `gpt-4o-mini` target the cascade is unchanged; ensemble mode never adds it. In the benchmark it covers the llama-target cases.
- **Expensive-output attacks tell the judge that any substantial attempt is success.** The judge's success list for every attack tagged `expensive_output` opens with a line saying correctness and completeness are irrelevant, because the harm is the resources spent (BM-0073). A tag, not the LLM04 category: applied category-wide it misfired on the Unicode bomb, a parsing attack (BM-0074).
- **Attack library 1.5.0.** `PS-LLM04-001`, `-002`, `-003` and `-005` are tagged `expensive_output`; `PS-LLM04-004` is not.
